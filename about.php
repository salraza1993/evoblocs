<?php include('includes/top-html.php') ?>
	<title>Welcome to Eveblocs | About Us</title>
	<?php include("includes/css.php") ?>
</head>
<body>
<!-- Header Begin -->
	<?php include('includes/header.php') ?>
<!-- Header End -->

<!-- Hero Banner Begins -->
  <section class="aboutTopSection">
    <?php include('includes/lineContainer.php') ?>
    <div class="container">
      <div class="row">
        <div class="offset-lg-3 col-12 col-lg-9 p-0 mb-5">
          <div class="topHeading">
            <h2>About Us</h2>
            <h3 class="recoleta-medium mb-4">
              <span  span class="recoleta-light">A</span>
              <span class="recoleta-semiBold primaryColor">Next-Gen Company</span>
              <span class="recoleta-light">dedicated to</span>
              <span class="recoleta-semiBold primaryColor">aligning digital</span>
              <span class="recoleta-light">with</span>
              <span class="recoleta-semiBold primaryColor">Your Business Goals</span>
            </h3>
          </div>          
        </div>
        <div class="d-flex p-0 align-items-center flex-wrap">
          <div class="col-12 col-md-12 col-lg-7 p-0 pe-5">
            <div class="ctaSection">
              <h3>
                We deliver <span class="recoleta-semiBold">revenue-generating digital marketing services</span> and <span class="recoleta-semiBold">technology platforms.</span>
              </h3>
              <button type="button" class="ctaButtonContainer mt-3 ctaButton whiteButton" buttonval="Let's get started">Let's get started</button>
            </div>
          </div>
          <div class="col-12 col-md-12 col-lg-5 p-0">
            <div class="aboutContent">
              <!-- <h2 class="mb-3">
                <span class="recoleta-light">It’s always about</span>
                <span class="recoleta-medium primaryColor">‘You’</span>
                <span class="recoleta-light">at</span>
                <span class="recoleta-medium primaryColor">EvoBlocs!</span>
              </h2> -->
              <p>
                While our clients may be different, they all share the need to have digital solutions built to generate revenue streams, establish communication channels, and streamline business operations. Our team of dedicated tech-savvy enthusiasts, designers, strategists, writers, and marketing experts work together leveraging our knowledge base, research, and experience of 15+ years in the industry, to do what we love, deliver innovative well-designed solutions to fit your needs, your business, and your budget expectations.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
<!-- Hero Banner Ends -->

<section class="howDoWeWorkSection py-4 mb-5">
  <?php include('includes/lineContainer.php') ?>
  <div class="container p-0">
    <div class="howDoWeWork__container">
      <div class="sectionHeading d-flex justify-content-between align-items-center p-0 mb-5">
        <div class="headingText">
          <p class="smallText text-uppercase roboto-light">working process</p>
          <h2>
            <span class="lightText">How</span> 
            <span class="primaryColor">We Work</span>
          </h2>
        </div>
      </div>
      <div class="arrodion__container">
        <div class="d-flex flex-wrap">
          <div class="col-12 col-lg-6 pe-5">
            <div class="arrodionImage">
              <img src="https://images.unsplash.com/photo-1542289539-059dea254938?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1964&q=80" alt="">
            </div>
          </div>
          <div class="col-12 col-lg-6">
            <div class="accordion">
              <div class="accordion__block active">
                <div class="accordion__header">
                  <h3>Research</h3>
                  <i class="fas fa-arrow-down"></i>
                </div>
                <div class="accordion__body" style="display: block;">
                  <p>
                    We understand your story, business, industry, products, services, and target audience - Your point of distinction and valuation.
                  </p>
                </div>
              </div>
              <div class="accordion__block">
                <div class="accordion__header">
                  <h3>Idea & Concept</h3>
                  <i class="fas fa-arrow-down"></i>
                </div>
                <div class="accordion__body">
                  <p>
                    We clarify your concept and employment stages, including the best applications of resources and time. 
                  </p>
                </div>
              </div>
              <div class="accordion__block">
                <div class="accordion__header">
                  <h3>Analyze & Optimization</h3>
                  <i class="fas fa-arrow-down"></i>
                </div>
                <div class="accordion__body">
                  <p>
                    We get a clear picture of our performance and reassess to drive desired results, keeping a clean record of where we’ve been, where we’re going and the technologies and strategies we’ve employed to get there.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

<section class="ourCoreVal__section py-5 mt-5">
  <?php include('includes/lineContainer.php') ?>
  <div class="container p-0">
    <div class="ourCoreVal__container">
      <div class="sectionHeading d-flex justify-content-between align-items-center p-0 mb-5 mt-5">
        <div class="headingText">
          <h2>
            <span class="lightText">Our</span> 
            <span class="primaryColor">Core Values</span>
          </h2>
        </div>
      </div>
    </div>
  </div>
  <div class="card__main__container horizontal__scroll__container text-center">
    <div class="card__container">
      <div class="card__icon">
        <img src="images/initiative.svg" alt="">
      </div>
      <div class="card__body">
        <h3>Initiative</h3>
        <p>
          We are proactive and go the extra mile for every project, regardless of size or number. You can see it in our work.
        </p>
      </div>
    </div>
    <div class="card__container">
      <div class="card__icon">
        <img src="images/creativity.svg" alt="">
      </div>
      <div class="card__body">
        <h3>Creativity</h3>
        <p>
          If digital is in our DNA, then creativity is in our blood. We constantly challenge the status quo through out-of-the-box processes, solutions, and design.
        </p>
      </div>
    </div>
    <div class="card__container">
      <div class="card__icon">
        <img src="images/knowledge.svg" alt="">
      </div>
      <div class="card__body">
        <h3>Knowledge</h3>
        <p>
          We employ some of the industry’s brightest minds, giving us an edge in identifying the most effective and efficient solutions for any business challenge.
        </p>
      </div>
    </div>
    <div class="card__container">
      <div class="card__icon">
        <img src="images/initiative.svg" alt="">
      </div>
      <div class="card__body">
        <h3>Transparency</h3>
        <p>
          We’re completely transparent from ideation to reporting. You’ll receive timely updates on all processes and insights for collaborative decision-making. 
        </p>
      </div>
    </div>
    <div class="card__container">
      <div class="card__icon">
        <img src="images/reliability.svg" alt="">
      </div>
      <div class="card__body">
        <h3>Reliability</h3>
        <p>
          Our performance is evaluated by holding our reliability as the benchmark. We will work together through every stage of development to achieve your business goals.
        </p>
      </div>
    </div>
    <div class="card__container">
      <div class="card__icon">
        <img src="images/quality.svg" alt="">
      </div>
      <div class="card__body">
        <h3>Quality</h3>
        <p>
          We derive our sense of fulfillment from the delivery and maintenance of the highest standards of work, which are on concentrating and constructing the foundational elements that propel businesses to win marketplaces. 
        </p>
      </div>
    </div>
  </div>
</section>

<!-- Home Latest Technologies section Begin -->
	<section class="latestTechnoSection">
  	<?php include('includes/lineContainer.php') ?>
		<div class="container">
			<div class="row ">
				<?php include('includes/latestTechnologies.php') ?>
			</div>
		</div>
	</section>
<!-- Home Latest Technologies section End -->

<section class="missionSection py-5">
  <?php include('includes/lineContainer.php') ?>
  <div class="container p-0">
    <div class="misson__container py-5">
      <div class="row align-items-center">
        <div class="col-12 col-lg-6 pe-0">
          <div class="sectionHeading d-flex justify-content-between align-items-center p-0 mb-4">
            <div class="headingText">
              <h2>
                <span class="lightText">Our</span> 
                <span class="primaryColor">Mission</span>
              </h2>
            </div>
          </div>
          <h3 class="pe-5 recoleta-light">
            We are committed to impacting
            <span class="recoleta-semiBold">brands</span> & <span class="recoleta-semiBold">businesses</span> 
            by enabling their 
            <span class="recoleta-semiBold">digital journeys</span> 
            through strategic solutions and innovative 
            <span class="recoleta-semiBold">designs, leveraging the appropriate technologies and practices</span> 
            to offer a 
            <span class="recoleta-semiBold">unique value proposition.</span>
          </h3>
          <button class="ctaButtonContainer ctaButton mt-5" buttonval="Let's discuss your project">Let's discuss your project</button>
        </div>
        <div class="col-6 col-lg-6 p-0 hidden-xs">
          <div class="missionIllustration">
            <img src="images/our-misson-illustration.svg" alt="">
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

<section class="joinTeamSection py-5">
  <?php include('includes/lineContainer.php') ?>
  <div class="container">
    <div class="row">
      <div class="col-12 col-md-6">
        <a href="team.php" class="joinTeamContainer __member my-5">
          <h3>Our Team</h3>
          <span>Members</span>
        </a>
      </div>
      <div class="col-12 col-md-6">
        <a href="career.php" class="joinTeamContainer __career my-5">
          <h3>Our Team</h3>
          <span>Career</span>
        </a>
      </div>
    </div>
  </div>
</section>

<!-- footer begin -->
<?php include('includes/footer.php') ?>
<!-- footer End -->